...
import dev.xesam.android.quickdemo.SimpleFilter;

/**
 * A simple {@link Fragment} subclass.
 * Activities that contain this fragment must implement the
 * {@link DemoListFragment.OnFragmentInteractionListener} interface
...
public class DemoListFragment extends Fragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        QuickDemo.inflateDemo(getActivity(), R.id.quick_demo_root, new DemoFilter(getContext()));
    }
    ...
    public class DemoFilter extends SimpleFilter {

        Pattern target = Pattern.compile("demo|sample|example", Pattern.CASE_INSENSITIVE);
        Pattern target2 = Pattern.compile("activity|fragment", Pattern.CASE_INSENSITIVE);
        String pkgName;
        ...
        public boolean filter(String className) {
            ...
            return  className.contains(".demo.")
                    ...
                    && target.matcher(simpleClassName).find()
                    && target2.matcher(simpleClassName).find()
